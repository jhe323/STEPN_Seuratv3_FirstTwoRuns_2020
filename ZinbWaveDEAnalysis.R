...
r1_zinb <- zinbwave(r1.sce, K = 20, epsilon=1000000, normalizedValues=T, residuals=T)
r2_zinb <- zinbwave(r2.sce, K = 20, epsilon=1000000, normalizedValues=T, residuals=T)

W.1 <- reducedDim(r1_zinb)
W.2 <- reducedDim(r2_zinb)
...
dds <- DESeqDataSet(r1_zinb, design = ~ Phase)

dds <- DESeq(dds, sfType="poscounts", useT=TRUE, minmu=1e-6)
res <- lfcShrink(dds, contrast=c("Phase"),
                 type = "normal")
head(res)

library(DESeq2)

dds <- DESeqDataSet(r2_zinb, design = ~ Phase)
...
df_zinb <- zinbwave(df.sce, K = 20, epsilon=1000000,normalizedValues=T,residuals=T)
...
dds <- DESeqDataSet(df_zinb, design = ~ Phase)

dds <- DESeq(dds, sfType="poscounts", useT=TRUE, minmu=1e-6)
res <- lfcShrink(dds, contrast=c("Phase", "NPC", "GW16"),
                 type = "normal")
head(res)
